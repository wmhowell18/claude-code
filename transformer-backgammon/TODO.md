...
- [x] **Fix Colab TPU kernel crash (SIGSEGV) in training notebook** — The install cell in `colab_tpu_training.ipynb` reinstalled `jax[tpu]` from the Cloud TPU wheel index, replacing Colab's preinstalled runtime-matched JAX/libtpu pair and causing a native stack-overflow segfault in the TPU compiler at training start. Install cell now keeps the preinstalled JAX, pins its version during dependency installs, and installs the package with `--no-deps`. Added troubleshooting row for kernel crashes. (Jul 2026)
